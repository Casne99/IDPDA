...
\end{prop}
    ...
    \begin{proof}
        ...
        $$\displaystyle d(L, x) \leq d(z, x) \leq \sum_{i=1}^{\lvert x \rvert} \gamma(\epsilon \rightarrow x_i) + \sum_{i=1}^{\lvert z \rvert} \gamma(z_i \rightarrow \epsilon) \leq \alpha \lvert x \rvert + \beta$$
    ...
    \begin{prop}
        Dato un linguaggio $L \in \Sigma^*$ ed una funzione di costo $\gamma$ esistono costanti $\alpha', \beta'$ tali che
        $\forall{x \in \Sigma^*}, \forall{y \in L}$ tali che $d(L, x) = d(y, x)$ vale $\lvert y \rvert \leq \alpha' \lvert x \rvert + \beta'$
    \end{prop}

    \begin{proof}
        Scegliamo $y \in L$ tale che $d(L, x) = d(y, x)$ e definiamo $\lambda$ come il minimo tra i costi degli inserimenti e delle cancellazioni:

        $$ \lambda = \min\bigl\{\{\gamma(\epsilon \rightarrow c) \mid c \in \Sigma\} \cup \{\gamma(c \rightarrow \epsilon) \mid c \in \Sigma\}\bigr\}$$
        ...
        ad almeno la differenza in lunghezza tra le due stringhe, segue quindi che $d(y, x) \geq \lambda\bigl\lvert(\lvert y \rvert - \lvert x \rvert)\bigr\rvert$.
        Si può quindi esprimere la disuguaglianza in termini di $y$:

        $$\lvert y \rvert \leq \lvert x \rvert + \frac{1}{\lambda}d(y, x)$$

        Senza perdita di generalità si può assumere $\lvert y \rvert - \lvert x \rvert \geq 0$.

        Usando la Proposizione~\ref{prop:distanza-limitata} e ricordando che $d(L, x) = d(y, x)$ otteniamo:

        $$\lvert y \rvert \leq (\frac{\alpha}{\lambda} + 1)\lvert x \rvert + \frac{\beta}{\lambda}$$
