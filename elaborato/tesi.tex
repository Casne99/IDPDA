...
% OR gates
    ...
    \subsection{$\text{co-NTIME}(\log n)$}
    \begin{definition}
        La classe $\text{co-NTIME}(\log n)$ è l'insieme dei linguaggi riconoscibili da macchine di Turing non deterministiche che operano in tempo $O(f(n))$ e tali che ogni stringa accettata ha almeno un cammino di computazione accettante.

        Più precisamente, un linguaggio $L$ appartiene a $\text{co-NTIME}(\log n)$ se esiste una macchina di Turing non deterministica $M$ tale che:
        \begin{itemize}
        ...
        La classe $CONTIME$ è l'unione di tutte le classi $\text{co-NTIME}(\log n)$ per funzioni $f$ calcolabili.
    \end{definition}
    ...
    \begin{theorem}
        Esiste un linguaggio context-sensitive $L$ tale che $P = NP$ se e solo se la distanza di edit di $L$ è calcolabile in tempo polinomiale
    \end{theorem}
    \begin{proof}
    ...
    La distanza di Levenshtein tra due stringhe $x$ ed $y$ è il numero minimo di operazioni di edit che consentono di trasformare $x$ in $y$.
