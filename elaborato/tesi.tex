...
\usepackage{amstext}            % Per \text{} in ambiente matematico
\usepackage{amsopn}             % Per \arg\min e operatori simili
...
\begin{document}
    ...
    Gli \emph{Input Driven Pushdown Automata}~\cite{okhotin-salomaa}, anche detti IDPDA, sono una classe di automi a pila in cui le operazioni sulla pila sono esclusivamente determinate dal simbolo di input.
    ...
    Questo conduce naturalmente al cosiddetto \textit{correction problem}.

    Dato un linguaggio $L$ e una stringa $x \in \Sigma^*$, il problema della correzione consiste non solo nel calcolare la distanza di edit tra $x$ e $L$, ma anche nel determinare esplicitamente una stringa $y \in L$ tale che tale distanza sia minima.


    Formalmente, si tratta quindi di risolvere un problema di ottimizzazione: trovare

    \[
        y = \arg\min_{w \in L} d(x,w)
    \]

    Il \textit{correction problem} richiede la costruzione effettiva di una correzione ottimale.
    In ambito applicativo, esso riveste particolare importanza in contesti quali il parsing robusto, il riconoscimento di input affetti da rumore e i sistemi di correzione automatica.
    ...
    Una transizione di un IDPDA è determinata dal tipo di simbolo in input, appartenente all'alfabeto $\Sigma$, che stabilisce l'operazione da eseguire sulla pila, eventualmente nulla.
    ...
    Se ci sono solo simboli neutri nell'alfabeto $\left(\Sigma_{+1} = \Sigma_{-1} = \emptyset\right)$, allora un NIDPDA è un automa a stati finiti non deterministico (NFA)
    ...
    Il primo dei modelli studiati sono state le \textit{grammatiche di parentesi} di McNaughton~\cite{mcnaughton}, nel quale l'alfabeto conteneva una parentesi sinistra $<$, una parentesi destra $>$
    ...
    Tutte queste grammatiche danno origine a sottoclassi dei linguaggi input-driven.
    ...
    Un modello più generale, studiato da Berstel e Boasson~\cite{berstel-boasson}, sono le \textit{grammatiche bilanciate}, nelle quali esiste una corrispondenza biunivoca tra parentesi sinistre e destre, in modo che ogni regola $A \rightarrow <\alpha>$ debba utilizzare una coppia di parentesi corrispondenti.
    ...
    Una certa stringa $y$ può essere derivata da un'altra stringa $x$ tramite una operazione di edit $a \rightarrow b$, se $\exists v, w \in \Sigma^*$ tale che $x = vaw, y = vbw$.
    ...
    La \textit{distanza di Hamming} è definita solo per stringhe di uguale lunghezza e considera esclusivamente operazioni di sostituzione, pertanto le operazioni di cancellazione ed inserimento hanno costo infinito.
    ...
    \begin{proof}
        ...
        \[
            \displaystyle d\left(L, x\right) \leq d\left(z, x\right) \leq \sum_{i=1}^{|x|} \gamma(\epsilon \rightarrow x_i) + \sum_{i=1}^{|z|} \gamma(z_i \rightarrow \epsilon) \leq \alpha |x| + \beta
        \]
        Definendo $\alpha = \max\{\gamma(\epsilon \rightarrow b) \mid b \in \Sigma\}$ e $\beta = \sum_{i=1}^{|z|} \gamma(z_i \rightarrow \epsilon)$ la dimostrazione è conclusa.
    \end{proof}
    ...
    \subsection{\textit{k}-vicinato}

    Esiste una nozione di distanza di \textit{k}-vicinato, date una distanza di edit ed un linguaggio, definita come l'insieme di tutte le stringhe a distanza al più \textit{k} da un qualche elemento in quel linguaggio.
    La famiglia dei linguaggi riconosciuti da un IDPDA è chiusa rispetto al \textit{k}-vicinato, ovvero se $L$ è un linguaggio riconosciuto da un IDPDA, allora anche il suo \textit{k}-vicinato è riconosciuto da un IDPDA. \\
    La dimostrazione~\cite{neighborhood} prende in esame un linguaggio $L$ su un un alfabeto $\Sigma = \Sigma_{-1} \cup \Sigma_{0} \cup \Sigma_{+1}$ riconosciuto da un IDPDA, e costruisce
    un altro IDPDA che riconosce il vicinato del linguaggio $L$, trattando separatamente i sei casi di inserimento e cancellazione per ogni tipo di simbolo dell'alfabeto (parentesi sinistre, destre e simboli neutri).
    Includere l'operazione di sostituzione inserirebbe ulteriori nove casi da trattare, ognuno identificato dalle classi del simbolo sostituto e sostituito.
    L'inclusione di questa operazione non cambia comunque il risultato ottenuto, la chiusura rispetto al \textit{k}-vicinato è mantenuta anche considerando operazioni di sostituzione.

    \subsubsection{Distanza di edit per automi input driven}

    Sia $\Sigma$ un alfabeto e $a \in \Sigma$ un simbolo.
    Dato un linguaggio $L \subseteq \Sigma^*$, l'insieme di tutte le stringhe ottenuto inserendo $a$ in qualunque posizione, per qualunque stringa in $L$, è definito come:
    \[
        ins_a(L) = \{\, u a v \mid u v \in L \,\}.
    \]

    Analogamente, l'insieme di tutte le stringhe ottenuto cancellando $a$ da qualunque posizione, per qualunque stringa in $L$, è definito come:
    \[
        del_a(L) = \{\, u v \mid u a v \in L \,\}.
    \]

    L'insieme di tutte le stringhe a distanza al più $k$ da una stringa $w$ è chiamato \textit{k}-vicinato di $w$ e si indica con $E_k(w)$ ed è definito come:

    \begin{align*}
        E_0(w) &= \{w\}, \\
        E_{k+1}(w) &= E_k(w) \cup \bigcup_{w' \in E_k\left(w\right)} \bigcup_{a \in \Sigma} \left(ins_a(w') \cup del_a(w')\right)
    \end{align*}

    Il \textit{k}-vicinato di un linguaggio $L$ è definito come l'unione dei \textit{k}-vicinati di tutte le stringhe in $L$:
    \[
    ...
    L'esistenza o meno di un algoritmo deterministico in tempo polinomiale per la risoluzione generale di \textsc{SAT} è nodo centrale del problema aperto $P \stackrel{?}{=} NP$.
    ...
    \begin{definition}
        La classe $AC^0$ è l'insieme dei linguaggi riconoscibili da famiglie uniformi di circuiti booleani di profondità costante e dimensione polinomiale, utilizzando porte AND, OR e NOT con fan-in illimitato.

        Formalmente, un linguaggio $L \subseteq \{0,1\}^*$ appartiene ad $AC^0$ se esiste una famiglia uniforme di circuiti $\{C_n\}_{n \geq 0}$ tale che:
        \begin{itemize}
...
% OR gates
    ...
    Le definizioni appena date sono particolarmente rilevanti nel contesto delle distanze di edit, poiché permettono di caratterizzare con precisione la complessità computazionale dei problemi considerati e di stabilire connessioni con le principali questioni aperte della teoria della complessità.
    ...
    \begin{proof}
        ...
        \noindent dove $\alpha'$ e $\beta'$ sono le costanti date in~\ref{prop:stringa-limitata}. \\
    ...
    \begin{proof}[Dimostrazione]
        ...
        Supponiamo che la distanza di edit verso \(L\) sia calcolabile in tempo polinomiale.
        Allora, dato un input \(x\), possiamo decidere in tempo polinomiale se \(x \in L\) osservando che
        \[
            x \in L \iff d(x,L) = 0,
        \]
        o, in altre parole, il calcolo della distanza di edit tra \(x\) e \(L\) generalizza il problema dell’appartenenza di \(x\) a \(L\). \\
        Ne consegue che \(L \in P\). \\
        Essendo \(L\) \(PSPACE\)-completo, segue che
        \[
            PSPACE \subseteq P,
        \]
        e quindi
    ...
    Come mostrato da Pighizzini~\cite{pighizzini} la distanza di edit per i problemi in questa classe è calcolabile in tempo polinomiale se e solo se $P = NP$.
    ...
    \[
        L_{\text{acc}} = \left\{
                             ...
                             \begin{array}{l}
                                 ...
                                 \forall i = 0,\dots,p(n),\; \omega_i \text{ è una configurazione di } M, \\[4pt]
                                 \forall i = 0,\dots,p(n),\; \lvert \omega_i \rvert = p'(n), \; \omega_i \in \left(\Gamma \cup Q \right)^* \\[4pt]
    ...
    Inoltre è stato osservato che il linguaggio $L_{acc}$ è accettato da un automa pushdown deterministico con nastro di input bidirezionale.
    Segue quindi che:

    \begin{prop}
        Esiste un linguaggio $L$ accettato da un automa pushdown deterministico con nastro di input bidirezionale tale che che $P = NP$ se e solo se
        la distanza di edit di $L$ è calcolabile in tempo polinomiale.
    \end{prop}

    \subsection{Calcolo efficiente della distanza di edit}

    Di seguito sono riportati alcuni risultati riguardanti il calcolo della distanza di edit per alcune classi di linguaggi. \\
    Aho e Peterson~\cite{aho-peterson} hannno dimostrato che la distanza di edit per i linguaggi context-free è calcolabile in tempo $O(n^3)$, dove $n$ è la lunghezza della stringa di input.
    La distanza di edit dei linguaggi regolari è calcolabile in tempo $O(n)$~\cite{wagner}. \\
    Gli algoritmi citati funzionano sotto l'assunzione che il costo delle operazioni di edit sia unitario. \\
    Pighizzini~\cite{pighizzini} mostra un algoritmo parallelo con complessità temporale $O(\log n)$ che utilizza un numero polinomiale di processori per calcolare
    le distanze di edit per una classe di linguaggi posta tra i linguaggi context-free ed i linguaggi context-sensitive, ovvero la classe dei linguaggi accettati da specifici automi input-driven non deterministici 1-NAuxPDA~\cite{brandenburg}. \\
    Quest'ultima è la più grande classe di linguaggi per cui è noto un algoritmo efficiente per il calcolo della distanza di edit.
    ...
    In questo elaborato abbiamo analizzato il problema del calcolo della distanza di edit rispetto a diverse classi di linguaggi, mettendo in relazione il problema stesso con gli Input Driven Pushdown Automata.
    Dopo aver introdotto il modello degli automi, nelle versioni deterministica e non deterministica, abbiamo mostrato come la struttura dell’alfabeto in simboli di \textit{push}, \textit{pop} e simboli neutri consenta di descrivere linguaggi con struttura annidata e corretta parentesizzazione, mantenendo al contempo una semantica operativa chiara.

    La seconda parte del lavoro ha formalizzato la nozione di distanza di edit tra stringhe e tra stringa e linguaggio, mettendo in evidenza il legame con il \textit{correction problem}: non solo misurare quanto un input sia distante dal linguaggio, ma anche individuare la stringa del linguaggio a distanza minima da quella data.
    In questo contesto sono stati richiamati risultati utili a delimitare la ricerca di soluzioni ottimali, come i vincoli lineari sulla lunghezza delle stringhe candidate e la nozione di \textit{k}-vicinato.

    Infine, la trattazione ha collocato il problema in un quadro di complessità più ampio.
    I risultati discussi mostrano che l’efficienza del calcolo della distanza di edit dipenda in modo profondo dalla classe del linguaggio considerato: accanto a casi trattabili con algoritmi noti, emergono connessioni dirette con questioni fondamentali come $P \stackrel{?}{=} NP$ e $P \stackrel{?}{=} PSPACE$.
    Abbiamo inoltre richiamato risultati consolidati, tra cui l’individuazione della più ampia classe di linguaggi per cui sia noto un algoritmo efficiente per il calcolo della distanza di edit, vale a dire la classe dei linguaggi accettati da automi 1-NAuxPDA\@.
    Questo aspetto evidenzia come il problema della distanza di edit non sia soltanto uno strumento applicativo per la correzione di input affetti da rumore, ma anche un punto di incontro tra teoria degli automi, parsing e teoria della complessità.

    \clearpage
    \begin{thebibliography}{9}
        \bibitem{okhotin-salomaa}
        Alexander Okhotin, Kai Salomaa,
        \textit{Complexity of Input-Driven Pushdown Automata, ACM SIGACT News, June 2014, vol. 45, no. 2}.
        \bibitem{berstel-boasson}
        J. Berstel, L. Boasson,
        \textit{“Balanced grammars and their languages”, Formal and Natural Computing 2002, LNCS 2300, 3-25}.
        \bibitem{pighizzini}
        Giovanni Pighizzini,
        \textit{How Hard Is Computing the Edit Distance? Information and Computation 165, 1-13 (2001)}.
        \bibitem{Karp-1}
        Karp, R. (1972),
        \textit{Reducibility among combinatorial problems, in “Complexity of Computer Computations” (R. Miller and J. Thatcher, Eds.), pp. 85-103, Plenum Press, New York}.
        \bibitem{mcnaughton}
        R. McNaughton,
        \textit{Parenthesis grammars, Journal of the ACM, 14:3 (1967), 490-500}.
        \bibitem{neighborhood}
        Alexander Okhotin, Kai Salomaa,
        \textit{Edit distance neighbourhoods of input-driven pushdown automata, Theoretical Computer Science 777 (2019), 417-430}.
        \bibitem{aho-peterson}
        Aho, A., Peterson, T. (1972),
        \textit{A minimum distance error-correcting parser for context free languages, SIAM J. Comput. 1, 305-312}.
        \bibitem{wagner}
        Wagner, R. (1974),
        \textit{Order-n correction for regular languages, Comm. Assoc. Comput. Mach. 17, 265-268}.
        \bibitem{brandenburg}
        Brandenburg, F. (1977),
        \textit{On one-way auxiliary pushdown automata, in “Proc. 3rd GI Conference”, Lecture Notes in Computer Science, Vol. 48, pp. 133-144, Springer-Verlag, Berlin/New York}.
    \end{thebibliography}
