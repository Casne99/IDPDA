...
\begin{document}
    ...
    \begin{figure}[H]
        \centering
        \begin{tikzpicture}[>=stealth, shorten >=1pt, auto, node distance=3.5cm, semithick, font=\scriptsize, scale=1.2, transform shape]
            \tikzstyle{every state}=[fill=white, draw=black, text=black, minimum size=12mm]
            ...
            (q1) edge[out=300, in=120] node[above right] {$)$ / pop $A$} (q2)
            (q1) edge[out=330, in=150] node[below right] {$]$ / pop $B$} (q2)
