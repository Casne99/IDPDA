...
\begin{document}
    ...
    Esiste una nozione rilevante di distanza di \textit{k}-vicinato, date una distanza di edit ed un linguaggio, definita come l'insieme di tutte le stringhe a distanza al più \textit{k} da un qualche elemento in quel linguaggio.
    ...
    La dimostrazione~\cite{neighborhood} prende in esame un linguaggio su un un alfabeto $\Sigma = \Sigma_{-1} \cup \Sigma_{0} \cup \Sigma_{+1}$ riconosciuto da un IDPDA, e costruisce
    un altro IDPDA che riconosce il vicinato di quel linguaggio, trattando separatamente i sei casi di inserimento, cancellazione e sostituzione per ogni tipo di simbolo dell'alfabeto (rispettivamente parentesi sinistre, destre e simboli neutri).
...
% OR gates
    ...
    le distanze di edit per una classe di linguaggi posta tra i linguaggi context-free ed i linguaggi context-sensitive, ovvero la classe dei linguaggi accettati da automi 1-NAuxPDA~\cite{brandeburg}.
